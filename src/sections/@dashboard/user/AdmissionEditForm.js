...
const AdmissionEditForm = (props) => {
  ...
  useEffect(() => {
    ...
    setdata({ ...rowDataWithoutCourse, course: course.split(',') });
  }, [props.editrow.row]);
  ...
  const handleSubmit = (e, _id) => {
    ...
    // Format the course data before sending
    const formattedCourse = Array.isArray(data.course) ? data.course.join(',') : data.course;
    ...
    axios
      .put(`https://academia-master-api.onrender.com/api/students/${data._id}`, requestData)
      .then((r) => {
